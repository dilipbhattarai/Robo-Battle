...
int activate();
int deactivate();
int thresholdRGB();// to initialize the RGB values of the 5 colors given
...
int nlabels;
target user1, user2, opponent1, opponent2, obstacle1, obstacle2;
...
int attack(){
	...
	//teta_e: angle of the opponent wrt the back of our robot
	//teta_u: angle of our robot
	//teta_o1: angle of obstacle1 1 wrt back of our  
	//difference_e_u: difference between teta_e and teta_u
	//difference_o_u: difference between teta_o1 and teta_u
	//dist: distance between user and opponent
	//dist_o1_f: distance between obstacle1 1 and front of our robot
	//dist_o1_b: distance between obstacle1 1 and back of our robot
	//alfa_e: vision angle between front of opponent and back of our robot based on distance
	//alfa_o: back of our robot vision angle that's blocked by obstacle1

	double teta_e, teta_u, dist, difference_e_u, difference_o_u, teta_o1, dist_o1_f, dist_o1_b,  alfa_e, alfa_o;

	draw_centroids();

	teta_e = (atan2(opponent1.jPos - user2.jPos, opponent1.iPos - user2.iPos)) * 180 / PI;
	teta_u = (atan2(user1.jPos - user2.jPos, user1.iPos - user2.iPos)) * 180 / PI;
	teta_o1 = (atan2(obstacle1.jPos - user2.jPos, obstacle1.iPos - user2.iPos)) * 180 / PI;
	...
	if (dist_o1_f < 80){
	 ...
	 if (dist_o1_b < 80)
		...
		if (abs(teta_e - teta_o1) < alfa_o){ // opponent is hiding behind obstacle1
			if (abs(difference_e_u) < 50) {
		...
		else { // there's no obstacle1 between us
			if (abs(difference_e_u) > alfa_e) {
...
//Initializes the RGB values for the colors used
int thresholdRGB(){
...
void draw_centroids(){
	...
	draw_point_rgb(rgb, (int)user2.iPos, (int)user2.jPos, 0, 0, 255);
	draw_point_rgb(rgb, (int)opponent1.iPos, (int)opponent1.jPos, 0, 255, 0);
	draw_point_rgb(rgb, (int)opponent2.iPos, (int)opponent2.jPos, 255, 255, 0);
	draw_point_rgb(rgb, obstacle1.iPos, obstacle1.jPos, 255, 0, 255);
